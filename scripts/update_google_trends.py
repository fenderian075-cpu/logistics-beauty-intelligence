...
def main():
    ...
    for i in range(0,len(terms),4):
        group=terms[i:i+4]; keywords=[anchor]+[x["term"] for x in group]
        try:
            ...
            for item in group:
                ...
                observations.append({
                    "term":term,"category":item.get("category","other"),"brand":item.get("brand"),
                    "interest_7d":round(recent7,1),"interest_prev21d":round(prev21,1),
                    "change_ratio":round(ratio,2),"change_pct":round((ratio-1.0)*100,1),
                    "anchor_normalized":round(recent28/anchor_mean,2),"source":"Google Trends","geo":"JP","confidence":"medium"
                })
            time.sleep(1.2)
    ...
    if not observations:
        old["last_attempt"]=now; old["collector_status"]="error"; old["collector_errors"]=errors
    ...
    result={
      "schema_version":"1.1","updated_at":now,"collector_status":"ok" if not errors else "partial",
      "source":{"name":"Google Trends","method":"pytrends","official_api":False,"geo":"JP","timeframe":"today 3-m","note":"Google Trendsの公開データを非公式クライアントpytrends経由で取得。指数は相対値であり検索件数ではありません。"},
      "anchor":anchor,"observations":observations,"signals":signals,"collector_errors":errors}
