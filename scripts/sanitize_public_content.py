#!/usr/bin/env python3
"""Sanitize public LBI content for Japanese-first UI and brand-target privacy.

The public repository must not expose individual beauty-brand monitoring targets,
brand-specific URLs, or mixed English/Japanese operational prose. Individual
monitoring targets belong outside the public data layer.
"""
...
# Japanese prose normalization. Proper source names and unavoidable technical
# acronyms (WCI, SCFI, IACI, NACCS, EPA/FTA, GDP etc.) are intentionally kept.
PHRASE_REPLACEMENTS = [
...
def normalize_text(text: str) -> str:
    out = text
    # Collapse explicit individual brand sequences to a public aggregate.
    pattern = "|".join(sorted((re.escape(x) for x in BRAND_TERMS), key=len, reverse=True))
    out = re.sub(pattern, "個別ブランド", out, flags=re.IGNORECASE)
    out = re.sub(r"(?:個別ブランド\s*[、,・/]\s*)+個別ブランド", "複数ブランド", out)
    for old, new in PHRASE_REPLACEMENTS:
        out = out.replace(old, new)
    # Common English tokens only when they appear as standalone words in Japanese prose.
    word_map = {
        "Daily": "日次", "Weekly": "週次", "Monthly": "月次", "campaign": "施策",
        "lane": "航路", "rate": "運賃", "space": "スペース", "spot": "スポット",
        "renewal": "契約更新", "outlook": "見通し", "risk": "リスク",
    }
...
def is_brand_url(value: object) -> bool:
    if not isinstance(value, str):
        return False
    low = value.lower()
    return any(domain in low for domain in BRAND_DOMAINS)
...
def sanitize_obj(value):
    ...
    if isinstance(value, list):
        ...
        # De-duplicate identical evidence records created by brand aggregation.
        if all(isinstance(v, dict) for v in result):
            seen, deduped = set(), []
            for item in result:
...
def sanitize_source_matrix() -> None:
    ...
    for source in data.get("sources", []):
        ...
        source["layer"] = LAYER_JA.get(layer, layer)
        source["extract"] = [EXTRACT_JA.get(x, normalize_text(x)) for x in source.get("extract", [])]
        # Hide all individual beauty-company / brand monitoring endpoints from the public ledger.
        if source.get("domain") == "beauty" and (is_brand_url(source.get("url")) or source.get("layer") in {"ブランド公式サイト監視", "企業情報"}):
...
def replace_priority_brand_file() -> None:
    path = ROOT / "data/beauty-priority-brands.json"
    data = {
...
def replace_buzz_watchlist() -> None:
    path = ROOT / "data/buzz-watchlist.json"
    data = {
        "schema_version": "2.0-public",
        "anchor": "化粧品",
        "public_note": "個別ブランド検索語は公開しない。公開版はカテゴリ・市場・販促テーマのみ保持する。",
        "terms": [
            {"term": "香水", "category": "フレグランス"},
            {"term": "リップ", "category": "メイクアップ"},
            {"term": "ファンデーション", "category": "メイクアップ"},
            {"term": "クッションファンデ", "category": "メイクアップ"},
            {"term": "美容液", "category": "スキンケア"},
            {"term": "日焼け止め", "category": "スキンケア"},
            {"term": "韓国コスメ", "category": "市場"},
            {"term": "チャーム コスメ", "category": "販促"},
            {"term": "ポーチ コスメ", "category": "販促"},
            {"term": "ギフト コスメ", "category": "販促"},
            {"term": "限定 コスメ", "category": "販促"},
            {"term": "GWP コスメ", "category": "販促"},
...
def sanitize_public_text_files() -> None:
    files = [
        ...
        ROOT / "assets/js/core/labels.js", ROOT / "assets/js/pages/source-matrix.js",
        ROOT / "assets/js/pages/economic-flow.js", ROOT / "assets/js/render/industry-deflator-panel.js",
        ROOT / "assets/js/render/market-regime.js", ROOT / "assets/js/render/market-regime-panel.js",
...
def main() -> None:
    ...
    sanitize_json_files()
    sanitize_public_text_files()
    print("Public content sanitized: Japanese-first vocabulary and individual brand targets removed.")
