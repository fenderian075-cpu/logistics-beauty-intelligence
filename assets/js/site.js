...
(function () {
  ...
  function emptyPanel(titleKey, type) {
    var panel = el("div", "panel");
    panel.appendChild(panelHead(titleKey, null));
    var body = el("div", "panel__body");
    if (type) body.appendChild(typeExplainer(type));
    body.appendChild(el("p", "muted", LBI.t("home.noReport")));
    panel.appendChild(body);
    return panel;
  }

  /** §4 — one-line "what is this report for" above every latest-report panel. */
  function typeExplainer(type) {
    var box = el("div", "type-explainer");
    box.appendChild(el("p", "type-explainer__q", LBI.t("type." + type + "Q")));
    box.appendChild(el("p", "type-explainer__d", LBI.t("type." + type + "Desc")));
    return box;
  }
  ...
  function renderDaily(report) {
    if (!report) return emptyPanel("home.latestDaily", "daily");
    var panel = el("div", "panel");
    panel.appendChild(panelHead("home.latestDaily", report));
    var body = el("div", "panel__body");
    body.setAttribute("data-translate", "");
    body.appendChild(typeExplainer("daily"));
  ...
  function renderWeekly(report) {
    if (!report) return emptyPanel("home.latestWeekly", "weekly");
    var panel = el("div", "panel");
    panel.appendChild(panelHead("home.latestWeekly", report));
    var body = el("div", "panel__body");
    body.setAttribute("data-translate", "");
    body.appendChild(typeExplainer("weekly"));
  ...
  function renderMonthly(report) {
    if (!report) return emptyPanel("home.latestMonthly", "monthly");
    var panel = el("div", "panel");
    panel.appendChild(panelHead("home.latestMonthly", report));
    var body = el("div", "panel__body");
    body.setAttribute("data-translate", "");
    body.appendChild(typeExplainer("monthly"));
  ...
  /* =======================================================================
     v2.1 dashboard — decision header, what changed, lenses, key signals.
     Everything below is derived at render time. No report object is mutated.
     ======================================================================= */

  var S = window.LBISignals;

  function renderActionBox(report, digest) {
    var box = document.getElementById("action-box");
    if (!box) return;
    box.innerHTML = "";
    var state = digest ? digest.actionRequired : "unknown";
    box.setAttribute("data-action", state);

    var labelKey = {
      required: "home.actionRequired",
      monitor: "home.actionMonitor",
      none: "home.actionNone",
      unknown: "home.actionUnknown"
    }[state];
    var noteKey = {
      required: "home.actionRequiredNote",
      monitor: "home.actionMonitorNote",
      none: "home.actionNoneNote",
      unknown: "home.actionUnknownNote"
    }[state];

    box.appendChild(el("p", "eyebrow", "Action"));
    var value = el("p", "action-box__value");
    value.appendChild(el("span", "dot"));
    value.appendChild(document.createTextNode(LBI.t(labelKey)));
    box.appendChild(value);
    box.appendChild(el("p", "action-box__note", LBI.t(noteKey)));
  }
  ...
  function renderConclusion(report) {
    ...
    if (!box) return;
    box.innerHTML = "";
    var text = report && (report.bottom_line || report.summary);
    if (!text) { box.hidden = true; return; }
    box.hidden = false;
    box.appendChild(el("p", "eyebrow", LBI.t("home.decisionTitle")));
    box.appendChild(el("p", null, text));
  }

  /* ---- WHAT CHANGED ------------------------------------------------------ */

  function renderWhatChanged(report, previous, digest) {
    var countsBox = document.getElementById("changed-counts");
    var listBox = document.getElementById("changed-list");
    var baseNote = document.getElementById("changed-base");
    if (!countsBox || !listBox) return;
    countsBox.innerHTML = "";
    listBox.innerHTML = "";
    if (baseNote) baseNote.textContent = "";

    var cs = (report && report.change_summary) || null;
    var base = cs && (cs.comparison_base || cs.compared_with);
    if (base && report && base === report.id) base = null;   // self-reference
    if (baseNote && base) {
      var target = data.reports.filter(function (r) { return r.id === base; })[0];
      if (target && target.id !== report.id) {
        var a = el("a", null, LBI.t("changes.comparedWith") + ": " + formatDate(target.date));
        a.href = root() + target.path;
        baseNote.appendChild(a);
      } else {
        baseNote.textContent = LBI.t("changes.comparedWith") + ": " + base;
      }
    } else if (baseNote && previous) {
      var pa = el("a", null, LBI.t("changes.comparedWith") + ": " + formatDate(previous.date));
      pa.href = root() + previous.path;
      baseNote.appendChild(pa);
    }

    // Legacy path: no structured signals — reuse the v1.1 comparison block.
    if (!digest || !digest.signals.length) {
      listBox.appendChild(renderChanges(report, previous));
      return;
    }

    S.CHANGE_PRIORITY.forEach(function (kind) {
      var n = digest.counts[kind] || 0;
      var chip = el("div", "change-count");
      chip.setAttribute("data-change", kind);
      chip.appendChild(el("span", "change-count__n", String(n)));
      chip.appendChild(el("span", "change-count__label", LBI.t("change." + kind)));
      if (!n) chip.classList.add("is-zero");
      countsBox.appendChild(chip);
    });

    if (!digest.changed.length) {
      listBox.appendChild(el("p", "empty-state", LBI.t("home.changedNone")));
      return;
    }

    var top = digest.changed.slice(0, 5);
    top.forEach(function (sig) {
      listBox.appendChild(S.card(sig, { reports: data.reports, rootPath: root(), compact: true }));
    });
    if (digest.changed.length > top.length) {
      listBox.appendChild(el("p", "muted small",
        LBI.t("home.changedMore", { n: digest.changed.length - top.length })));
    }
  }
  ...
  function renderLenses(digest) {
    ...
    var note = document.getElementById("lenses-note");
    if (!grid) return;
    grid.innerHTML = "";

    if (note) {
      var empty = !digest || !digest.signals.length;
      note.hidden = !empty;
      if (empty) note.textContent = LBI.t("home.noIntelligence");
    }

    S.LENSES.forEach(function (lens) {
      var bucket = digest ? digest.byLens[lens] : { status: "unconfirmed", count: 0, top: null };
      var card = el("div", "lens-card");
      card.setAttribute("data-status", bucket.status);
      card.setAttribute("data-lens", lens);

      var head = el("div", "lens-card__head");
      head.appendChild(el("span", "lens-card__name", LBI.t("lens." + lens)));
      var pill = el("span", "status-pill lens-state-pill");
      pill.setAttribute("data-lens-state", bucket.status);
      pill.appendChild(el("span", "dot"));
      pill.appendChild(document.createTextNode(LBI.t("lensState." + bucket.status)));
      head.appendChild(pill);
      card.appendChild(head);

      card.appendChild(el("p", "lens-card__count",
        bucket.count ? LBI.t("home.lensCount", { n: bucket.count }) : LBI.t("home.lensEmpty")));

      if (bucket.top) {
        var top = el("p", "lens-card__top", bucket.top.signal || S.signalName(bucket.top));
        top.setAttribute("data-translate", "");
        card.appendChild(top);
        var badges = el("div", "lens-card__badges");
        var b1 = el("span", "badge badge--change");
        b1.setAttribute("data-value", bucket.top.change_status || "unchanged");
        b1.textContent = LBI.t("change." + (bucket.top.change_status || "unchanged"));
        badges.appendChild(b1);
        var b2 = el("span", "badge badge--conf");
        b2.setAttribute("data-value", bucket.top.confidence || "low");
        b2.textContent = LBI.t("sig.confidence") + ": " + LBI.t("conf." + (bucket.top.confidence || "low"));
  ...
  function renderKeySignals(digest) {
    var box = document.getElementById("key-signals");
    if (!box) return;
    box.innerHTML = "";
    if (!digest || !digest.key.length) {
      box.appendChild(el("p", "empty-state", LBI.t("home.noIntelligence")));
      return;
    }
    digest.key.forEach(function (sig) {
      box.appendChild(S.card(sig, { reports: data.reports, rootPath: root() }));
    });
  }

  function renderAll() {
    var daily = latestOf("daily");
    var previous = daily ? LBIData.previousOf(data.reports, daily) : null;
    var digest = daily ? S.summarise(daily) : null;

    renderOverall(daily);
    renderActionBox(daily, digest);
  ...
  // The registry is optional: signals.js degrades to inline metadata if the
  // fetch fails, so a registry problem must never block the dashboard.
  Promise.all([loadReports(), S.loadRegistry(root())]).then(function (results) {
    data = results[0];
    renderAll();
