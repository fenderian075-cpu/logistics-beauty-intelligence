...
import { observations, latest, yoyOf, seriesOf, unitLabelOf } from "./economy.js";
...
function buildRegistry(bundle) {
  ...
  Object.entries(bundle || {}).forEach(([key, dataset]) => {
    seriesOf(dataset).forEach((series) => {
      ...
      const candidate = { key, dataset, series };
      const current = registry.get(series.metric_id);
...
function statsFor(series, rows) {
  const values = rows.map((row) => Number(row.value)).filter(Number.isFinite);
  const last = rows[rows.length - 1] || null, first = rows[0] || null;
  if (!values.length || !last) return null;
  const min = Math.min(...values), max = Math.max(...values), mean = average(values), med = median(values), latestValue = Number(last.value);
  const rangePosition = max === min ? 50 : ((latestValue - min) / (max - min)) * 100;
  const averageGap = mean && mean !== 0 ? ((latestValue / mean) - 1) * 100 : null;
  const firstValue = first ? Number(first.value) : null;
  const totalChange = Number.isFinite(firstValue) && firstValue !== 0 ? ((latestValue / firstValue) - 1) * 100 : null;
  return { min, max, mean, median: med, rangePosition, averageGap, totalChange, last, first, turn: turningPoint(rows), count: rows.length };
}

function statCard(label, value, note) {
  const card = el("div", "history-stat");
  card.appendChild(el("span", "history-stat__label", label));
  card.appendChild(el("strong", "history-stat__value", value || "—"));
  if (note) card.appendChild(el("span", "history-stat__note", note));
  return card;
}

function formatSeriesValue(series, value) {
  return value == null || !Number.isFinite(Number(value)) ? UNKNOWN : formatObservation(series.unit, Number(value)).text;
}

function latestMom(row) {
  if (!row) return null;
  return row.mom == null ? null : Number(row.mom);
}
...
function analysisNotes(series, rows, stats) {
  if (!stats) return [];
  const notes = [];
  const current = stats.last, yoy = yoyOf(current), mom = latestMom(current);
  if (yoy != null) notes.push(`前年比 ${formatPct(yoy)}で${directionalPhrase(yoy, "上昇", "低下")}。`);
  if (mom != null) notes.push(`前月比 ${formatPct(mom)}で${directionalPhrase(mom, "上向き", "下向き")}。`);
...
function historyTable(series, rows) {
  const details = el("details", "history-table");
  details.appendChild(el("summary", "history-table__toggle", `観測値を表示（${rows.length}件）`));
  const wrap = el("div", "table-scroll"), table = el("table", "data-table history-table__table"), thead = el("thead"), hr = el("tr");
  ["期間", "値", "前年比", "前月比", "区分"].forEach((label) => { const th = el("th", null, label); th.setAttribute("scope", "col"); hr.appendChild(th); });
  thead.appendChild(hr); table.appendChild(thead);
  const tbody = el("tbody");
  rows.slice().reverse().forEach((row) => {
    const tr = el("tr"), yoy = yoyOf(row), mom = latestMom(row);
    [formatPeriod(row.period), formatSeriesValue(series, row.value), yoy == null ? "—" : formatPct(yoy), mom == null ? "—" : formatPct(mom), row.status || "—"].forEach((value) => tr.appendChild(el("td", null, value)));
...
function sourceNode(entry) {
  ...
  if (source) {
    if (source.url) {
      const a = el("a", "history-source__link", source.name || "一次情報"); a.href = source.url; a.target = "_blank"; a.rel = "noopener noreferrer"; meta.appendChild(a);
...
function createShell() {
  ...
  const body = el("div", "history-panel__body"); panel.appendChild(body); document.body.appendChild(backdrop); document.body.appendChild(panel);
...
function updateUrl(metricId) {
  try {
    ...
    if (metricId) url.searchParams.set("metric", metricId); else url.searchParams.delete("metric");
    window.history.replaceState(null, "", `${url.pathname}${url.search}${url.hash}`);
  } catch (_) { /* static file / embedded preview */ }
}

function renderEntry(entry, shell, windowKey) {
  const { series, dataset } = entry, allRows = observations(series), def = WINDOWS.find((item) => item.key === windowKey) || WINDOWS[0], rows = rowsForWindow(allRows, def.years), stats = statsFor(series, rows), current = latest(series);
  clear(shell.body);
  const head = el("header", "history-summary"); head.appendChild(el("h3", "history-summary__metric", metricName(series))); head.appendChild(sourceNode(entry));
  head.appendChild(el("p", "history-summary__period", allRows.length ? `${formatPeriod(allRows[0].period)} 〜 ${formatPeriod(allRows[allRows.length - 1].period)} · ${allRows.length}観測` : "観測値なし")); shell.body.appendChild(head);

  const controls = el("div", "history-window"); controls.setAttribute("aria-label", "表示期間");
  WINDOWS.forEach((item) => { const button = el("button", "history-window__button", item.label); button.type = "button"; button.setAttribute("data-history-window", item.key); button.setAttribute("aria-pressed", item.key === def.key ? "true" : "false"); controls.appendChild(button); });
  ...
  statGrid.appendChild(statCard("前年比", current && yoyOf(current) != null ? formatPct(yoyOf(current)) : "—", "同月・同期間比較"));
  statGrid.appendChild(statCard("前月比", current && latestMom(current) != null ? formatPct(latestMom(current)) : "—", "月次系列のみ"));
  ...
  statGrid.appendChild(statCard("表示期間", stats ? `${stats.count}観測` : "—", stats ? `${formatPeriod(stats.first.period)} → ${formatPeriod(stats.last.period)}` : "—"));
  shell.body.appendChild(statGrid);
  ...
  if (figure) { figure.classList.add("history-chart"); shell.body.appendChild(figure); }
  else shell.body.appendChild(el("p", "chart-absent", "推移グラフを描くための観測が不足しています。"));

  const notes = analysisNotes(series, rows, stats); if (notes.length) { const section = el("section", "history-reading"); section.appendChild(el("h4", "history-reading__title", "データから読めること")); const list = el("ul", "history-reading__list"); notes.forEach((note) => list.appendChild(el("li", null, note))); section.appendChild(list); section.appendChild(el("p", "history-reading__caution", "数値の位置と変化を要約したもので、因果関係を推定するものではありません。")); shell.body.appendChild(section); }
...
function enhanceTargets(registry) {
  document.querySelectorAll("[data-history-metric]").forEach((node) => {
    ...
    if (!node.hasAttribute("aria-haspopup")) node.setAttribute("aria-haspopup", "dialog");
    if (!node.title) node.title = "クリックして長期推移を分析";
  });
}

export function mountHistoryExplorer(bundle) {
  const registry = buildRegistry(bundle), shell = createShell();
  let activeMetric = null, activeWindow = "all", lastFocus = null;
  enhanceTargets(registry);

  const open = (metricId, { updateHistory = true } = {}) => {
    const entry = registry.get(metricId); if (!entry) return;
    activeMetric = metricId; activeWindow = "all"; lastFocus = document.activeElement;
    renderEntry(entry, shell, activeWindow); shell.backdrop.hidden = false; shell.panel.hidden = false; document.documentElement.setAttribute("data-history-open", "true");
    if (updateHistory) updateUrl(metricId); shell.close.focus();
  };
