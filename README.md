...
| ファイル | 役割 |
|---|---|
| `index.html` | ダッシュボード（Layer 1）— 判断 → レーダー → ステータス → 最新レポート → 変化 → 市場レジーム → 視点 → トピック |
| `radar.html` | オペレーションレーダー（Layer 1.5）— critical-news の全件。状態優先の並び、URL に保存される絞り込み |
| `topic.html?id=` | Topic Intelligence Digest（Layer 2）— テーマ単位の深掘り。id なしで一覧 |
...
│
├── assets/
│   ├── css/{tokens,base,components,pages}.css   ← 実体
│   ├── css/{style,portal-v22,ui-fixes-v26,intelligence-v3}.css ← 互換シム
│   └── js/app.js + {core,data,domain,render,pages}/  ← ES Modules
│
├── data/
│   ├── reports.json
...
UI 側の扱い（`assets/js/domain/signals.js` の `directionTone()`）:
...
enum を増やす場合は、`data/signal-registry.json` の `meta.enums`、`assets/js/domain/signals.js`、`assets/js/core/labels.js`、`scripts/validate-report.py` の **4か所を同時に** 更新してください。
...
**UI は日本語のみです。** v5 で翻訳レイヤー（`assets/js/translation.js`、言語切替、`data-i18n` / `data-translate`）を全廃しました。

| 項目 | 現在の扱い |
|---|---|
| UI 文言・enum ラベル | `assets/js/core/labels.js`（日本語のみ。未知の値は値そのものを表示） |
| レポート本文 | 日本語の原文のみ。英語版 HTML は作りません |
| 英語で読みたい場合 | ブラウザ自身のページ翻訳機能を使ってください |

`scripts/validate-report.py` は `data-i18n` / `data-translate` / `lang-switch` を検出するとエラーにします。レポートは必ず `templates/report-template.html` から生成してください。

lens 名は英語併記をやめ、**障害 / コスト・キャパ / 定時性 / 需要・商流 / 規制・構造** に統一しています。
